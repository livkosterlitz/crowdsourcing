...
# plot -------------------------------------------------------
compare_means(Resistance ~ treatment_name, data = df %>% filter(focal == 'Ec1'), group.by = 'Rep', ref.group = "Ec1", method = "wilcox.test", p.adjust.method = "bonferroni")
...
E <- ggplot(data = df %>% filter(focal == 'Ec1'), aes(x=treatment_name, y=Resistance, color = treatment_name))+
  stat_summary(data = df %>% filter(focal == 'Ec1', Rep !=baseline), mapping = aes(treatment_name, Resistance, color = treatment_name), fun.data=MinMeanSEMMax, geom="boxplot", size = box_line_size)+ 
  geom_jitter(data = df %>% filter(focal == 'Ec1', Rep !=baseline), width = point_jitter_width, size = point_size, alpha = 0.1) +
  stat_summary(data = df %>% filter(focal == 'Ec1', Rep ==baseline), color = 'dark grey', mapping = aes(treatment_name, Resistance, color = treatment_name), fun.data=MinMeanSEMMax, geom="boxplot", size = box_line_size)+ 
  geom_jitter(data = df %>% filter(focal == 'Ec1', Rep ==baseline), color = 'dark grey', width = point_jitter_width, size = point_size, alpha = 0.1) +
  facet_grid(~Rep)+
...
compare_means(Resistance ~ treatment_name, data = df %>% filter(focal == 'Ec2'), group.by = 'Rep', ref.group = "Ec2", method = "wilcox.test", p.adjust.method = "bonferroni")
...
K <- ggplot(data = df %>% filter(focal == 'Ec2'), 
  ...
  stat_summary(data = df %>% filter(focal == 'Ec2', Rep != baseline), mapping = aes(treatment_name, Resistance, color = treatment_name), fun.data=MinMeanSEMMax, geom="boxplot", size = box_line_size)+ 
  geom_jitter(data = df %>% filter(focal == 'Ec2', Rep != baseline), width = point_jitter_width, size = point_size, alpha = 0.1) +
  stat_summary(data = df %>% filter(focal == 'Ec2', Rep == baseline), color = 'dark grey', mapping = aes(treatment_name, Resistance, color = treatment_name), fun.data=MinMeanSEMMax, geom="boxplot", size = box_line_size)+ 
  geom_jitter(data = df %>% filter(focal == 'Ec2', Rep == baseline), color = 'dark grey', width = point_jitter_width, size = point_size, alpha = 0.1) +
  facet_grid(~Rep)+
...
compare_means(Resistance ~ treatment_name, data = df %>% filter(focal == 'Ec3'), group.by = 'Rep', ref.group = "Ec3", method = "wilcox.test", p.adjust.method = "bonferroni")

Se.max <- as.numeric(df %>% filter(focal == 'Ec3') %>% summarise(maxR = max(Resistance)))
S <- ggplot(data = df %>% filter(focal == 'Ec3'), 
            aes(x=treatment_name, y=Resistance, color = treatment_name))+
  stat_summary(data = df %>% filter(focal == 'Ec3', Rep !=baseline), mapping = aes(treatment_name, Resistance, color = treatment_name), fun.data=MinMeanSEMMax, geom="boxplot", size = box_line_size)+ 
  geom_jitter(data = df %>% filter(focal == 'Ec3', Rep !=baseline), width = point_jitter_width, size = point_size, alpha = 0.1) +
  stat_summary(data = df %>% filter(focal == 'Ec3', Rep ==baseline), color = 'dark grey', mapping = aes(treatment_name, Resistance, color = treatment_name), fun.data=MinMeanSEMMax, geom="boxplot", size = box_line_size)+ 
  geom_jitter(data = df %>% filter(focal == 'Ec3', Rep ==baseline), color = 'dark grey', width = point_jitter_width, size = point_size, alpha = 0.1) +
...
save_plot("SI_Fig12.pdf", plot = Fig, base_width = 6.5, base_height = 7.5)
