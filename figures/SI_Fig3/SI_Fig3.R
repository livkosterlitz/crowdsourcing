...
# plot -------------------------------------------------------
compare_means(Resistance ~ treatment_name, data = df %>% filter(focal == 'Ec'), group.by = 'time_step', ref.group = "Ec", method = "wilcox.test", p.adjust.method = "bonferroni")
...
E <- ggplot(data = df %>% filter(focal == 'Ec'), 
  ...
  stat_summary(data = df %>% filter(focal == 'Ec', time_step !=baseline), mapping = aes(treatment_name, Resistance, color = treatment_name), fun.data=MinMeanSEMMax, geom="boxplot", size = box_line_size)+ 
  geom_jitter(data = df %>% filter(focal == 'Ec', time_step !=baseline), width = point_jitter_width, size = point_size, alpha = 0.1) +
  stat_summary(data = df %>% filter(focal == 'Ec', time_step ==baseline), color = 'dark grey', mapping = aes(treatment_name, Resistance, color = treatment_name), fun.data=MinMeanSEMMax, geom="boxplot", size = box_line_size)+ 
  geom_jitter(data = df %>% filter(focal == 'Ec', time_step ==baseline), color = 'dark grey', width = point_jitter_width, size = point_size, alpha = 0.1) +
...
compare_means(Resistance ~ treatment_name, data = df %>% filter(focal == 'Kp'), group.by = 'time_step', ref.group = "Kp", method = "wilcox.test", p.adjust.method = "bonferroni")

Kp.max <- as.numeric(df %>% filter(focal == 'Kp') %>% summarise(maxR = max(Resistance)))
K <- ggplot(data = df %>% filter(focal == 'Kp'), 
            aes(x=treatment_name, y=Resistance, color = treatment_name))+
  stat_summary(data = df %>% filter(focal == 'Kp', time_step !=baseline), mapping = aes(treatment_name, Resistance, color = treatment_name), fun.data=MinMeanSEMMax, geom="boxplot", size = box_line_size)+ 
  geom_jitter(data = df %>% filter(focal == 'Kp', time_step !=baseline), width = point_jitter_width, size = point_size, alpha = 0.1) +
  stat_summary(data = df %>% filter(focal == 'Kp', time_step ==baseline), color = 'dark grey', mapping = aes(treatment_name, Resistance, color = treatment_name), fun.data=MinMeanSEMMax, geom="boxplot", size = box_line_size)+ 
  geom_jitter(data = df %>% filter(focal == 'Kp', time_step ==baseline), color = 'dark grey', width = point_jitter_width, size = point_size, alpha = 0.1) +
  facet_grid(~time_step)+
...
compare_means(Resistance ~ treatment_name, data = df %>% filter(focal == 'Se'), group.by = 'time_step', ref.group = "Se", method = "wilcox.test", p.adjust.method = "bonferroni")

Se.max <- as.numeric(df %>% filter(focal == 'Se') %>% summarise(maxR = max(Resistance)))
S <- ggplot(data = df %>% filter(focal == 'Se'), 
            aes(x=treatment_name, y=Resistance, color = treatment_name))+
  stat_summary(data = df %>% filter(focal == 'Se', time_step !=baseline), mapping = aes(treatment_name, Resistance, color = treatment_name), fun.data=MinMeanSEMMax, geom="boxplot", size = box_line_size)+ 
  geom_jitter(data = df %>% filter(focal == 'Se', time_step !=baseline), width = point_jitter_width, size = point_size, alpha = 0.1) +
  stat_summary(data = df %>% filter(focal == 'Se', time_step ==baseline), color = 'dark grey', mapping = aes(treatment_name, Resistance, color = treatment_name), fun.data=MinMeanSEMMax, geom="boxplot", size = box_line_size)+ 
  geom_jitter(data = df %>% filter(focal == 'Se', time_step ==baseline), color = 'dark grey', width = point_jitter_width, size = point_size, alpha = 0.1) +
  facet_grid(~time_step)+
...
save_plot("SI_Fig4.pdf", plot = Fig, base_width = 6.5, base_height = 7.5)
