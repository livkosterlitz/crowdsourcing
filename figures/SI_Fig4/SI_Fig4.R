...
# DHFR landscape key --------------
draw.figure.DHFR.ancestor.key<-function(){
  df<-read.csv("inputs/ancestor.csv",header=TRUE)
  plot.new()
  draw.landscape.graph(df,
                       N.loci=3,
                       starting.position.labels=c("P","A","L"),
                       ending.position.labels=c("L","T","R"),
                       N.fit.estimates=1,
                       buffer.x=0.5,
                       buffers.y=c(0,0,0),
                       node.radius=0.4,
...
draw.figure.DHFR.evolved.key<-function(){
...
# DHFR landscape network graph --------------
draw.figure.DHFR.network <-function(network.x.y.ratio){
  df<-read.csv("inputs/DHFR.csv",header=TRUE)
  plot.new()
...
draw.figure.DHFR.network.blowups <-function(network.x.y.ratio,
                                            buffer.x = 0.05,
                                            buffer.y = c(0,0,0)){
  df<-read.csv("inputs/DHFR.csv",header=TRUE)
  plot.new()
  draw.landscape.blowups(df,
                         ...
                         starting.position.labels=c("","",""),
                         ending.position.labels=c("","",""),
                         starting.blowup.labels=c("P","A","L"),
                         ending.blowup.labels=c("L","T","R"),
                         N.fit.estimates=1,
                         buffer.x=buffer.x,
                         buffers.y=buffer.y,
                         node.radius=0.025,
...
sp.slopes.df <- read.csv("inputs/mutational_step_slopes_DHFR.csv")
sp.slopes <- sp.slopes.df %>%
  select(-X, -focal_MIC, -mutant_MIC, -focal_num, -mutant_num)
min(sp.slopes$step_slope)
max(sp.slopes$step_slope)

Ec1.main.label = expression('mutation effect in ' * phantom(' Ec1'))
Ec1.sp.label = expression(phantom('mutation effect in ') * ' Ec1')
Ec1.label = c(Ec1.main.label, Ec1.sp.label)

Ec2.main.label = expression('mutation effect in ' * phantom('Ec2'))
Ec2.sp.label = expression(phantom('mutation effect in ') * 'Ec2')
Ec2.label = c(Ec2.main.label,Ec2.sp.label)

Ec3.main.label = expression('mutation effect in ' * phantom('Ec3'))
Ec3.sp.label = expression(phantom('mutation effect in ') * 'Ec3')
Ec3.label = c(Ec3.main.label,Ec3.sp.label)

Ec1.Ec2.no.epistasis <- sp.slopes %>%
  filter(Species %in% c('Ec1','Ec2')) %>%
  pivot_wider(names_from = Species, values_from = c(step_slope,step_effect)) %>%
  filter(step_effect_Ec1 == step_effect_Ec2) %>%
  rename(Ec1 = step_slope_Ec1, Ec2 = step_slope_Ec2)

Ec1.Ec2.yes.epistasis <- sp.slopes %>%
  filter(Species %in% c('Ec1','Ec2')) %>%
  pivot_wider(names_from = Species, values_from = c(step_slope,step_effect)) %>%
  filter(step_effect_Ec1 != step_effect_Ec2) %>%
  rename(Ec1 = step_slope_Ec1, Ec2 = step_slope_Ec2)

Ec1.Ec3.no.epistasis <- sp.slopes %>%
  filter(Species %in% c('Ec1','Ec3')) %>%
  pivot_wider(names_from = Species, values_from = c(step_slope,step_effect)) %>%
  filter(step_effect_Ec1 == step_effect_Ec3) %>%
  rename(Ec1 = step_slope_Ec1, Ec3 = step_slope_Ec3)

Ec1.Ec3.yes.epistasis <- sp.slopes %>%
  filter(Species %in% c('Ec1','Ec3')) %>%
  pivot_wider(names_from = Species, values_from = c(step_slope,step_effect)) %>%
  filter(step_effect_Ec1 != step_effect_Ec3) %>%
  rename(Ec1 = step_slope_Ec1, Ec3 = step_slope_Ec3)

Ec2.Ec3.no.epistasis <- sp.slopes %>%
  filter(Species %in% c('Ec2','Ec3')) %>%
  pivot_wider(names_from = Species, values_from = c(step_slope,step_effect)) %>%
  filter(step_effect_Ec2 == step_effect_Ec3) %>%
  rename(Ec2 = step_slope_Ec2, Ec3 = step_slope_Ec3)

Ec2.Ec3.yes.epistasis <- sp.slopes %>%
  filter(Species %in% c('Ec2','Ec3')) %>%
  pivot_wider(names_from = Species, values_from = c(step_slope,step_effect)) %>%
  filter(step_effect_Ec2 != step_effect_Ec3) %>%
  rename(Ec2 = step_slope_Ec2, Ec3 = step_slope_Ec3)
...
correlation_plot <- function(df.no, df.yes, x.host, y.host, x.label, x.color, y.label, y.color, no.color, yes.color, global.cex, tick.cex = 1) {
  plot((-2:12), (-2:12), type="n", xlab='', ylab='', frame.plot=F, axes = F)
  ...
  lines(-2:12, -2:12, lwd = 1, col = "black")
}
...
fitness.landscape.graph <- function(host, 
  ...
  df.rel <- df %>%
    ...
    mutate(MIC_WT = fit.1[Num_of_mutations == 0],
           fit.1 = fit.1 - MIC_WT)
  df.segments <- read.csv('inputs/mutational_step_slopes_rel_DHFR.csv')
  df.host <- df.rel %>% filter(hosts == host)
  df.host.segments <- df.segments %>% filter(Species == host)
  n_mut <- max(df.rel$Num_of_mutations)
  plot(seq(-0.25, n_mut+0.25, length.out = 4), seq((min(df.rel$fit.1)-0.5),(max(df.rel$fit.1)+0.5),length.out = 4), type="n", xaxt='n', xlab='', ylab='', axes=FALSE, frame.plot=TRUE)
  ...
  ending.color=pie_color
  N.loci = 3
  axis.multiplier = ((max(df.rel$fit.1)+0.5)-(min(df.rel$fit.1)-0.5))/(N.loci+0.5)
  node.edge.color = 'black'
  node.edge.width = 0.5
  label.pos = 0.5
  node.label.size = 0
...
globalpointsize = 9
tickpointsize = 7
pdf(file = 'SI_Fig4.pdf', width = pdf.width, height = pdf.height, pointsize = pointsize)
...
draw.figure.DHFR.network.blowups(panel.A.x.y.ratio, buffer.y = c(0.1,0.1,0))
...
correlation_plot(df.no = Ec1.Ec2.no.epistasis, df.yes = Ec1.Ec2.yes.epistasis, x.host = 'Ec1', y.host = 'Ec2', x.label = Ec1.label, y.label = Ec2.label, x.color = red_Ec_color, y.color = blue_Kp_color, no.color = pie_arrow, yes.color = purple_Ec.Kp_color, global.cex = global.cex, tick.cex = tick.cex)
mtext("b", line = -1.1, side=3, at = -13)  
correlation_plot(df.no = Ec2.Ec3.no.epistasis, df.yes = Ec2.Ec3.yes.epistasis, x.host = 'Ec2', y.host = 'Ec3', x.label = Ec2.label, y.label = Ec3.label, x.color = blue_Kp_color, y.color = yellow_Se_color, no.color = pie_arrow, yes.color = green_Se.Kp_color, global.cex = global.cex, tick.cex = tick.cex)
correlation_plot(df.no = Ec1.Ec3.no.epistasis, df.yes = Ec1.Ec3.yes.epistasis, x.host = 'Ec3', y.host = 'Ec1', x.label = Ec3.label, y.label = Ec1.label, x.color = yellow_Se_color, y.color = red_Ec_color, no.color = pie_arrow, yes.color = orange_Ec.Se_color, global.cex = global.cex, tick.cex = tick.cex)
...
fitness.landscape.graph(host = 'Ec1',
...
fitness.landscape.graph(host = 'Ec3',
