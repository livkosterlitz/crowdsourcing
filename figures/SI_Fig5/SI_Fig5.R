...
simulation_plot_split <- function(df,
                                  ...
                                  buffer_proportion) {
df_early <- df %>% filter(Cumulative_time <= 12)
df_late <- df %>% filter(Cumulative_time > 24)
df_middle <- df %>% filter(Cumulative_time <= 24 & Cumulative_time > 12)
plot((0:x_max), seq(0, y_max, y_max/x_max), type="n", xlab='', ylab='', frame.plot=T, axes = F)
...
pdf.height = r5 + r6 + r6 +(outer_margin*2) # 9.25 max
### Initiate  --------
pointsize = 12
globalpointsize = 9
tickpointsize = 7
pdf(file = 'SI_Fig5.pdf', width = pdf.width, height = pdf.height, pointsize = pointsize)
layout(matrix(matrix_layout, nrow = n.row, ncol = n.col),
       widths = c(lcm(c1*2.54),lcm(c2*2.54),lcm(c3*2.54)), 
       heights = c(lcm(r5*2.54),lcm(r6*2.54),lcm(r6*2.54)))
...
df_Ec1_only <- read.csv("inputs/74_averages.csv")
df_Ec1_Ec2 <- read.csv("inputs/75_averages.csv")
df_Ec1_Ec3 <- read.csv("inputs/76_averages.csv")
Ec1_sim_ymax = 1400
simulation_plot(df = df_Ec1_only, 
                x.main.label = 'time', 
                ...
                x_max = 36, 
                y_max = Ec1_sim_ymax)
mtext("a", line = -0.5, side=3, at = -10)  
par(mai=c(r5_bottom_margin,c2_left_margin,r5_top_margin,c2_right_margin), cex = global.cex)
arrow_prop = 17.5
line_prop_mult = 4
buffer_prop = c(50, 50) # was 80
simulation_plot_split(df = df_Ec1_Ec2, 
                      df.mean = df_Ec1_only,
                      x.main.label = 'time', 
                      global.cex = global.cex, 
                      y.main.label = "", 
...
buffer_prop = c(-500, 50)
simulation_plot_split(df = df_Ec1_Ec3, 
                      df.mean = df_Ec1_only,
                      x.main.label = 'time', 
                      global.cex = global.cex, 
                      y.main.label = "", 
...
df_Ec2_only <- read.csv("inputs/92_averages.csv")
df_Ec2_Ec1 <- read.csv("inputs/93_averages.csv")
df_Ec2_Ec3 <- read.csv("inputs/94_averages.csv")
Ec2_sim_ymax = 1300

simulation_plot(df = df_Ec2_only, 
                x.main.label = 'time', 
                global.cex = global.cex, 
                y.main.label = "resistance", 
...
### Panel G, H, I---------
par(mai=c(r5_bottom_margin,c1_left_margin,r6_top_margin,c1_right_margin), cex = global.cex)
df_Ec3_only <- read.csv("inputs/110_averages.csv")
df_Ec3_Ec1 <- read.csv("inputs/111_averages.csv")
df_Ec3_Ec2 <- read.csv("inputs/112_averages.csv")
Ec3_sim_ymax = 1700

simulation_plot(df = df_Ec3_only, 
                x.main.label = 'time', 
                global.cex = global.cex, 
                y.main.label = "resistance", 
                ...
                x_max = 36, 
                y_max = Ec3_sim_ymax)
mtext("g", line = -0.5, side=3, at = -10)  
par(mai=c(r5_bottom_margin,c2_left_margin,r6_top_margin,c2_right_margin), cex = global.cex)
arrow_prop = 17.5
line_prop_mult = 4
buffer_prop = c(1000, 1000) # was 80
simulation_plot_split(df = df_Ec3_Ec1, 
                      df.mean = df_Ec3_only,
                      x.main.label = 'time', 
                      global.cex = global.cex, 
                      y.main.label = "", 
                      ...
                      line_proportion_multiplier = line_prop_mult,
                      buffer_proportion = buffer_prop)
mtext("h", line = -0.5, side=3, at = -5)  
par(mai=c(r5_bottom_margin,c3_left_margin,r6_top_margin,c3_right_margin), cex = global.cex)
buffer_prop = c(50, 50)
simulation_plot_split(df = df_Ec3_Ec2, 
                      df.mean = df_Ec3_only, 
                      x.main.label = 'time', 
                      global.cex = global.cex, 
                      y.main.label = "", 
