...
# mutation sweep data -------------------------------------------------------
df <- Parameter_sweep_load_data(treatments_filename = "mutation", 
...
# plot -------------------------------------------------------
compare_means(Resistance ~ treatment_name, data = df %>% filter(focal == 'Ec1'), group.by = 'mu', ref.group = "Ec1", method = "wilcox.test", p.adjust.method = "bonferroni")

Ec.max <- as.numeric(df %>% filter(focal == 'Ec1') %>% summarise(maxR = max(Resistance)))
E <- ggplot(data = df %>% filter(focal == 'Ec1'), aes(x=treatment_name, y=Resistance, color = treatment_name))+
  stat_summary(data = df %>% filter(focal == 'Ec1', mu !=baseline), mapping = aes(treatment_name, Resistance, color = treatment_name), fun.data=MinMeanSEMMax, geom="boxplot", size = box_line_size)+ 
  geom_jitter(data = df %>% filter(focal == 'Ec1', mu !=baseline), width = point_jitter_width, size = point_size, alpha = 0.1) +
  stat_summary(data = df %>% filter(focal == 'Ec1', mu ==baseline), color = 'dark grey', mapping = aes(treatment_name, Resistance, color = treatment_name), fun.data=MinMeanSEMMax, geom="boxplot", size = box_line_size)+ 
  geom_jitter(data = df %>% filter(focal == 'Ec1', mu ==baseline), color = 'dark grey', width = point_jitter_width, size = point_size, alpha = 0.1) +
  facet_grid(~mu)+
...
compare_means(Resistance ~ treatment_name, data = df %>% filter(focal == 'Ec2'), group.by = 'mu', ref.group = "Ec2", method = "wilcox.test", p.adjust.method = "bonferroni")
...
K <- ggplot(data = df %>% filter(focal == 'Ec2'), 
  ...
  stat_summary(data = df %>% filter(focal == 'Ec2', mu !=baseline), mapping = aes(treatment_name, Resistance, color = treatment_name), fun.data=MinMeanSEMMax, geom="boxplot", size = box_line_size)+ 
  geom_jitter(data = df %>% filter(focal == 'Ec2', mu !=baseline), width = point_jitter_width, size = point_size, alpha = 0.1) +
  stat_summary(data = df %>% filter(focal == 'Ec2', mu ==baseline), color = 'dark grey', mapping = aes(treatment_name, Resistance, color = treatment_name), fun.data=MinMeanSEMMax, geom="boxplot", size = box_line_size)+ 
  geom_jitter(data = df %>% filter(focal == 'Ec2', mu ==baseline), color = 'dark grey', width = point_jitter_width, size = point_size, alpha = 0.1) +
  facet_grid(~mu)+
...
compare_means(Resistance ~ treatment_name, data = df %>% filter(focal == 'Ec3'), group.by = 'mu', ref.group = "Ec3", method = "wilcox.test", p.adjust.method = "bonferroni")

Se.max <- as.numeric(df %>% filter(focal == 'Ec3') %>% summarise(maxR = max(Resistance)))
S <- ggplot(data = df %>% filter(focal == 'Ec3'), 
            aes(x=treatment_name, y=Resistance, color = treatment_name))+
  stat_summary(data = df %>% filter(focal == 'Ec3', mu !=baseline), mapping = aes(treatment_name, Resistance, color = treatment_name), fun.data=MinMeanSEMMax, geom="boxplot", size = box_line_size)+ 
  geom_jitter(data = df %>% filter(focal == 'Ec3', mu !=baseline), width = point_jitter_width, size = point_size, alpha = 0.1) +
  stat_summary(data = df %>% filter(focal == 'Ec3', mu ==baseline), color = 'dark grey', mapping = aes(treatment_name, Resistance, color = treatment_name), fun.data=MinMeanSEMMax, geom="boxplot", size = box_line_size)+ 
  geom_jitter(data = df %>% filter(focal == 'Ec3', mu ==baseline), color = 'dark grey', width = point_jitter_width, size = point_size, alpha = 0.1) +
...
save_plot("SI_Fig10.pdf", plot = Fig, base_width = 6.5, base_height = 7.5)
