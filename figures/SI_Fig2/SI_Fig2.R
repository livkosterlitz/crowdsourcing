...
# plot -------------------------------------------------------
compare_means(Resistance ~ treatment_name, data = df %>% filter(focal == 'Ec'), group.by = 'mu', ref.group = "Ec", method = "wilcox.test", p.adjust.method = "bonferroni")
...
E <- ggplot(data = df %>% filter(focal == 'Ec'), aes(x=treatment_name, y=Resistance, color = treatment_name))+
  stat_summary(data = df %>% filter(focal == 'Ec', mu !=baseline), mapping = aes(treatment_name, Resistance, color = treatment_name), fun.data=MinMeanSEMMax, geom="boxplot", size = box_line_size)+ 
  geom_jitter(data = df %>% filter(focal == 'Ec', mu !=baseline), width = point_jitter_width, size = point_size, alpha = 0.1) +
  stat_summary(data = df %>% filter(focal == 'Ec', mu ==baseline), color = 'dark grey', mapping = aes(treatment_name, Resistance, color = treatment_name), fun.data=MinMeanSEMMax, geom="boxplot", size = box_line_size)+ 
  geom_jitter(data = df %>% filter(focal == 'Ec', mu ==baseline), color = 'dark grey', width = point_jitter_width, size = point_size, alpha = 0.1) +
  facet_grid(~mu)+
...
compare_means(Resistance ~ treatment_name, data = df %>% filter(focal == 'Kp'), group.by = 'mu', ref.group = "Kp", method = "wilcox.test", p.adjust.method = "bonferroni")
...
K <- ggplot(data = df %>% filter(focal == 'Kp'), 
  ...
  stat_summary(data = df %>% filter(focal == 'Kp', mu !=baseline), mapping = aes(treatment_name, Resistance, color = treatment_name), fun.data=MinMeanSEMMax, geom="boxplot", size = box_line_size)+ 
  geom_jitter(data = df %>% filter(focal == 'Kp', mu !=baseline), width = point_jitter_width, size = point_size, alpha = 0.1) +
  stat_summary(data = df %>% filter(focal == 'Kp', mu ==baseline), color = 'dark grey', mapping = aes(treatment_name, Resistance, color = treatment_name), fun.data=MinMeanSEMMax, geom="boxplot", size = box_line_size)+ 
  geom_jitter(data = df %>% filter(focal == 'Kp', mu ==baseline), color = 'dark grey', width = point_jitter_width, size = point_size, alpha = 0.1) +
  facet_grid(~mu)+
...
compare_means(Resistance ~ treatment_name, data = df %>% filter(focal == 'Se'), group.by = 'mu', ref.group = "Se", method = "wilcox.test", p.adjust.method = "bonferroni")

Se.max <- as.numeric(df %>% filter(focal == 'Se') %>% summarise(maxR = max(Resistance)))
S <- ggplot(data = df %>% filter(focal == 'Se'), 
            aes(x=treatment_name, y=Resistance, color = treatment_name))+
  stat_summary(data = df %>% filter(focal == 'Se', mu !=baseline), mapping = aes(treatment_name, Resistance, color = treatment_name), fun.data=MinMeanSEMMax, geom="boxplot", size = box_line_size)+ 
  geom_jitter(data = df %>% filter(focal == 'Se', mu !=baseline), width = point_jitter_width, size = point_size, alpha = 0.1) +
  stat_summary(data = df %>% filter(focal == 'Se', mu ==baseline), color = 'dark grey', mapping = aes(treatment_name, Resistance, color = treatment_name), fun.data=MinMeanSEMMax, geom="boxplot", size = box_line_size)+ 
  geom_jitter(data = df %>% filter(focal == 'Se', mu ==baseline), color = 'dark grey', width = point_jitter_width, size = point_size, alpha = 0.1) +
...
save_plot("SI_Fig3.pdf", plot = Fig, base_width = 6.5, base_height = 7.5)
