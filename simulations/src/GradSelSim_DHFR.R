...
simulate.gradient.selection <- function(initial.genotype,
                                        final.genotype,
                                        landscape1.df,
                                        N.loci,
                                        neighborhoods,
                                        ...
                                        focal.host.name,
                                        sim.name) {
  endtimes <- rep(0,N.reps) # records the time when the final genotype is reached
  average.MIC <- rep(0,N.time.steps+1)
  traj1 <- matrix(NA,nrow=N.reps,ncol=N.time.steps) #OK added
  for(r in 1:N.reps) {
    chosen.genotype.1 <- initial.genotype
    first.time<-TRUE
    for(t in 0:N.time.steps) {
      traj1[r,t] <- chosen.genotype.1
      template.genotype <- chosen.genotype.1
      if(final.genotype == template.genotype) {
        if(first.time) {
          endtimes[r] <- t+1
          first.time <- FALSE
        }
      }
      average.MIC[t+1]<-average.MIC[t+1]+compute.mics(chosen.genotype.1,landscape1.df)
      chosen.genotype.1 <- pick.next.genotype(template.genotype,
                                              neighborhoods,
                                              landscape1.df,
                                              N.transformants,
                                              mu,
  ...
  df <- data.frame(Focal.host = focal.host.name,
                   Sim.condition = sim.name, 
                   Round = seq(0,N.time.steps), 
                   Ave.mic = average.MIC/N.reps)
  return(list(endtimes, df, traj1))
}
...
# Load landscapes --------
landscape_file <- read.csv("data/DHFR/DHFR_sim_file.csv")
landscapes <- landscape_file %>%
  rename(hosts = Species, mic = MIC) %>%
  select(hosts, P, A, L, mic)

id<-1:8
Ec1.landscape.df <- cbind(id,landscapes[landscapes$hosts=="Ec1",2:5])
Ec1.landscape.df$Species <- 'Ec1'
Ec2.landscape.df <- cbind(id,landscapes[landscapes$hosts=="Ec2",2:5])
Ec2.landscape.df$Species <- 'Ec2'
Ec3.landscape.df <- cbind(id,landscapes[landscapes$hosts=="Ec3",2:5])
Ec3.landscape.df$Species <- 'Ec3'
landscapes <- rbind(Ec1.landscape.df,
                    Ec2.landscape.df,
                    Ec3.landscape.df)

# Creation of the mutational neighbor matrix -------
N.g<-length(Ec1.landscape.df$id) # number of genotypes
...
for(f.gen in 1:N.g) { #loop through the number of genotypes
  ...
  for(o.gen in 1:N.g) { #loop through the number of genotypes
    ...
    if(mutational.distance(as.integer(Ec1.landscape.df[f.gen,2:(1+N.l)]),
                           as.integer(Ec1.landscape.df[o.gen,2:(1+N.l)]))==1) {
...
treatment_csv <- read.csv("data/DHFR/Treatment_master_DHFR.csv")
Treatment_ID <- NA
Cumulative_time <- NA
Species_time <- NA
Species_order <- NA
...
for(id in treatment_csv$Treatment_ID){
  ...
  for(r in 1:max(treatment_df$Replicate)) {
    ...
    treatment_df$P[row_marker] <- landscapes$P[landscape_row_marker]
    treatment_df$A[row_marker] <- landscapes$A[landscape_row_marker]
    treatment_df$L[row_marker] <- landscapes$L[landscape_row_marker]
  ...
  write.csv(x = treatment_df, file = paste("results/DHFR/", Treatment_ID[1], '.csv', sep = ''))
  write.csv(x = treatment_df_summary, file = paste("results/DHFR/", Treatment_ID[1], '_averages.csv', sep = ''))
}
